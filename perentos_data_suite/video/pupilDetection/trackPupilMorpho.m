...
varNames = {'radius','centerX','centerY','area','metric','interpFlag'};
...
%             prd = (1./sqrt( (frCx-centers(:,2)).^2 + (frCy-centers(:,1)).^2 )).*metric';
            ...
            subplot(1,3,3);
            imshow(bw); hold on;
            viscircles([centers(idx,1) centers(idx,2)],radii(idx));% viscircles([centers(2,1) centers(2,2)],radii(2));
            subplot(1,3,1);
...
if track & ~exist([pth,'/processed/','pupilRes.mat'])
    ...
    res = [res{:}];
    res = reshape(res,5,length(res)/5);
    res = res';
    resSm = res; 
    
%% find outliers using robust mad estimation, then linearly interpolate them
    resSm = res; close all;
    t = (1:size(res,1))'; 
    exAll = zeros(length(t),1);
    for i = 1:4
        Excluded = Isoutlier(res(:,i));
        exAll = (exAll | Excluded);
    end
    exAll([1,end])=0;
    s1 = find(exAll)-1;%s1(s1<1)=[];
    s2 = find(exAll)+1;%s2(s2>length(Excluded))=[];
    exAll([s1 s2]) = 1;
    
%     if exAll(1) == 1; exAll(1) = []; end
%     if exAll(end) == 1; exAll(end) = []; end
    nmes = {'radii','centerX','centerY','area',};
    for i = 1:4
        %Excluded = Isoutlier(res(:,i));
        resSm(exAll,i) = nan;
        nanx = isnan(resSm(:,i));      %resSm(:,i) = inpaint_nans(resSm(:,i)); % or  
        resSm(nanx,i) = interp1(t(~nanx), resSm(~nanx,i), t(nanx));
        %exAll = (exAll | Excluded);
        % 
        figure;%figure('pos',[1086 5 1100 700]);        
        subplot(3,2,[1 2]); plot(res(:,i));
        hold on; plot(find(Excluded),res(Excluded,i),'.r');
        subplot(3,2,[3 4]); plot(resSm(:,i)); 
        linkaxes(get(gcf,'children'),'x');  
        xlim([0 100*30]);

        subplot(3,2,5); histogram(resSm(:,i),200,'edgecolor','none');      title('''good'' datapoints');
        subplot(3,2,6); histogram(res(Excluded,i),200,'edgecolor','none'); title('''bad'' datapoints')      
        set(all_subplots(gcf),'linewidth',1, 'box','off','tickdir','out','XLimSpec', 'tight','YLimSpec', 'tight');
        mtit([varNames{i},', n_excl = ',num2str(sum(exAll))], 'interpreter','none');
        print([pth,'/processed/','pupil_',varNames{i},'.jpg'],'-djpeg','-r300');         
    end
    % a flag holding the indices of the interpolated values
    resSm(:,6) = exAll;
...
%%  save everything  
    ...
    % SAVE
    save([pth,'/processed/','pupilRes.mat'],'res','resSm','varNames');
    out.res = res;
    out.resSm = resSm;
...
%% detect outliers and interpolate them
% if 0
%     % this procedure uses a labbox function to identify outilers but its a bit
%     % too conservative
%     ps = [0.0001 0.0002 0.0003];
%     close all;
%     for pp = 1:1        
%         resSm = res;
%         for i = 1:4              
%             resSm(Excluded,i) = nan;
%             [Included, Excluded] = OutlierRemove(res(:,i), ps(pp));
%             nanx = isnan(resSm(:,i));        
%             resSm(nanx,i) = interp1(t(~nanx), resSm(~nanx,i), t(nanx));%resSm(:,i) = inpaint_nans(resSm(:,i)); % or
%             %
%             figure('pos',[1086 5 1100 700]);
%             subplot(3,2,[1 2]); plot(res(:,i));
%             hold on; plot(Excluded,res(Excluded,i),'.r');
%             subplot(3,2,[3 4]); plot(resSm(:,i));    
%             linkaxes(get(gcf,'children'),'x','y');
%             subplot(3,2,5); histogram(res(Included,i));
%             subplot(3,2,6); histogram(res(Excluded,i));    
%             mtit(['OutlierRemove:',varNames{i},', n_excl = ',num2str(length(Excluded)),'p<',num2str(ps(pp))],', interpreter','none');
%         end
%     end
% 
%     %% this procedure goes the simple way using zscores
%     close all;
%     for SD = 2%1:5
%         resSm = res;
%         for i = 1:4
%             z = zscore(resSm(:,i));
%             Excluded = find(z > SD | z < -SD );
%             resSm(Excluded,i) = nan;
%             nanx = isnan(resSm(:,i));      %resSm(:,i) = inpaint_nans(resSm(:,i)); % or  
%             resSm(nanx,i) = interp1(t(~nanx), resSm(~nanx,i), t(nanx));
%             % 
...
%             subplot(3,2,[3 4]); plot(resSm(:,i));     
%             linkaxes(get(gcf,'children'),'x');       
%             subplot(3,2,5); histogram(res(setxor(1:length(res),Excluded),1));
%             subplot(3,2,6); histogram(res(Excluded,1));
%             mtit(['zscore:',varNames{i},', n_excl = ',num2str(length(Excluded)),', z<',num2str(SD)],', interpreter','none');
%         end    
% 
% end
% end